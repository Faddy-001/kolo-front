...
@Component({
  selector: 'app-deactive-leads',
  templateUrl: './deactive-leads.component.html',
  styleUrls: ['./deactive-leads.component.scss']
})
export class DeactiveLeadsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.pageCounts = [];
    this.newLeads = [];
    this.mainservice.deActiveleads().subscribe((res)=>{
      this.Leads = res;
...
sortCategory(asc:boolean)
{
  ...
  this.newLeads.sort(function (a:any, b:any) 
  {
    ...
    //return a.Category.localeCompare(b.Category.rendered);
  ...
  Activate(id:any){
    this.mainservice.ActivateLead(id).subscribe((res)=>{
      this.activateRes = res;
      this.toastr.success(this.activateRes.message);
      this.ngOnInit();
    })
  }

  exportLead(){
    this.mainservice.exportLead('deactive')
  }
  ...
  getNextData(pageNo: number) {
    this.pageCounts = [];
    this.newLeads = [];
    this.mainservice.getDPagination(pageNo).subscribe((result) => {
      this.Leads = result;
      this.pageCount = this.Leads.totalPages;
      this.page = this.Leads.page;
      console.log(this.Leads);
  ...
  getNextDeactiveData(e: any) {
    this.activePage=e.target.value;
    this.pageCounts = [];
    this.newLeads = [];
    this.mainservice.getDPagination(e.target.value).subscribe((result) => {
    this.Leads = result;
    console.log(this.Leads+'hi');
 ...
 visitDate:any;
  ...
  openFilter() {
    const dialogRef = this.dialog.open(FilterComponent, {
      height:'800px',
      // width: '500px',
      data: {deactive:'deactive',project:this.selectedProject,date:this.visitDate,index:this.selectedindex,cre:this.selectedCre, salesExecutive:this.selectedSE,
